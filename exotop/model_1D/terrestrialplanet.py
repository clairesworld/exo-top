...
class TerrestrialPlanet():
    ...
    def __init__(self, **kwargs):
        ...
        default_attr = dict(
            ...
            CMF=0.325,  # needs a value to avoid errors
            ...
            kappa_m=None,  # if None then calculate from k, c_m
            kappa_lm=None,
    ...
    def init_derived(self, bug_Rc=False, verbose=False, **kwargs):
        ...
        if self.R_c0 is not None:
            print('setting R_c with R_c0', self.R_c0 * 1e-3, '- may not be consistent with input CMF')
            self.R_c = self.R_c0
            self.CRF = self.R_c0 / self.R_p0
        elif self.CMF is not None:
            if bug_Rc:
                if verbose:
        ...
        if self.CMF > 0:
            ...
            self.g_cmb = 0
        if self.kappa_m is None:
            self.kappa_m = thermal.thermal_diffusivity(self.k_m, self.rho_m, self.c_m)
        if self.kappa_lm is None:
            self.kappa_lm = thermal.thermal_diffusivity(self.k_lm, self.rho_m, self.c_m)
    ...
    def reinitialise(self, R_c=None, CMF=None, R_p=None):  # reinitialse from structure (e.g. better EoS
        self.R_p = R_p
        self.R_c = R_c
        self.CMF = CMF
        self.M_c = self.M_p * self.CMF
        self.CRF = self.R_c / self.R_p
        self.M_m = self.M_p - self.M_c
        self.d = self.R_p - self.R_c
        self.SA_p = geom.SA(R=self.R_p)
        self.SA_c = geom.SA(R=self.R_c)  # core surface area
        self.g_sfc = ast.grav(self.M_p, self.R_p)
