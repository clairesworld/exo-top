...
def Ra_F(pl=None, nu=None, eta=None, kappa=None, H=None, alpha=None, k=None, rho=None, g=None, l=None,
...
def ubl_thickness_beta(dT=None, d_m=None, Ra_crit=None, beta=None, g=None, Ra_rh=None,
...
def h_rad(t, tf=None, H_0=None, H_f=None, c_n=None, p_n=None, lambda_n=None, backwards_cooling=False, **kwargs):
...
def recalculate(t, pl, adiabats=0, complexity=3, Tlid_ini=None, **kwargs):
    ...
    if hasattr(pl, 'd_m_const'):  # if exploring a constant convecting region height
    ...
    # radiogenic heating
    pl.h_rad_m = h_rad(t, H_0=pl.H_0, H_f=pl.H_f, c_n=pl.c_n, p_n=pl.p_n, lambda_n=pl.lambda_n, **kwargs)  # W kg^-1
    pl.a0 = pl.h_rad_m * pl.rho_m  # radiogenic heating in W m^-3

    # check if stopped convecting (stagnant lid extends to core)
    if (not isinstance(pl.h_rad_m, Iterable)) and (pl.D_l > pl.R_p - pl.R_c):
        print(pl.M_p/p.M_E, 'M_E: convection stopped at', t*p.sec2Gyr, 'Gyr because lid grew to core')
    ...
    if convecting:
        ...
        if (not isinstance(pl.dT_m, Iterable)) and (pl.dT_m < 0):
            pl.dT_m = 1e-5  # avoid numerical errors
        ...
        if (not isinstance(pl.Ra_i_eff, Iterable)) and (abs(pl.Ra_i_eff) < 1e-9):
            print('REALLY SMALL: Ra', pl.Ra_i_eff, 'eta', pl.eta_m, 'deltaT', pl.dT_m, 'l', pl.d_m, 'T_c - T-l', pl.T_c, '-', pl.T_l)
        elif (not isinstance(pl.Ra_i_eff, Iterable)) and (pl.Ra_i_eff < 0):
            print('NEGATIVE: Ra', pl.Ra_i_eff, 'eta', pl.eta_m, 'deltaT', pl.dT_m, 'l', pl.d_m, 'T_c - T-l', pl.T_c, '-',
                  pl.T_l)

        pl.delta_rh = ubl_thickness_beta(d_m=pl.d_m, Ra_crit=pl.Ra_crit_u, beta=pl.beta_u, Ra_rh=pl.Ra_i_eff, **kwargs)
        ...
        if pl.SA_c > 0:  # if core included in model
        ...
        pl.q_sfc = sph_flux(pl.R_p, a0=pl.a0, T_l=pl.T_l, R_l=pl.R_l, k_m=pl.k_m, T_s=pl.T_s, R_p=pl.R_p,
                            **kwargs)  # W m^-2
        if Tlid_ini == 'linear':
        ...
        pl.d_m = 0
        pl.R_l = pl.R_c
        pl.T_l = T_lid(T_m=pl.T_m, a_rh=pl.a_rh, Ea=pl.Ea)  # this is just theoretical now
        pl.dT_rh = pl.a_rh * (p.R_b * pl.T_m ** 2 / pl.Ea)  # also just theoretical
        V_lid = 4 / 3 * np.pi * (pl.R_p ** 3 - pl.R_l ** 3)
        pl.M_lid = V_lid * pl.rho_m  # not considering density change in lid due to melting/differentiation
        pl.M_conv = 0
        ...
        pl.nu_m = pl.eta_m / pl.rho_m
        pl.nu_cmb = pl.eta_cmb / pl.rho_m
        pl.delta_eta = pl.eta_s / pl.eta_cmb  # total viscosity contrast
